...
#ifndef REDBLACKTREE_TEST_H_
#define REDBLACKTREE_TEST_H_

#include <iostream>
using namespace std;

#include "../../test.h"
#include "../../../include/sequential/binarytree/redblacktree.h"
using namespace yatl;

class RedBlackTreeTest : public Test {
public:
	const char *getName() const { return "Red-Black Tree"; }
	bool runTest() {
		RedBlackTree<int,int> bst;
		for (int i=0; i < 20*7; i += 7) {
			int k = i % 13;
			RedBlackTree<int,int>::NodeType *btn = new RedBlackTree<int,int>::NodeType(k,i);
			bst.insertNode(btn);
		}

		Assume(bst.getHeight() == 4, ("Height should be 4 not %d", bst.getHeight()));

		RedBlackTree<int,int>::NodeType *btn = bst.searchNode(6);
	    ...
	    RedBlackTree<int,int>::NodeType *succ = bst.getSuccessor(btn);
	    ...
	    RedBlackTree<int,int>::NodeType *pred = bst.getPredecessor(btn);
	    ...
	    // where we end up
	    RedBlackTree<int,int>::iterator it(bst,btn);
